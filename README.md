# Nomads-Bar
nomads drink menu updated 7/4/26 backup
...
<head>
    ...
    <style>
        ...
        #header-logo-container {
            ...
            background-image: 
                /* NEW: Added a subtle color gradient overlay to warm up the grey */
        ...
        #menu-grid {
            ...
            padding: 1rem 0 6.9rem; /* Reduced top/bottom padding */
            height: 100%; /* Removed arbitrary 52px subtraction */
        }
        ...
        .drink-card {
            ...
            min-height: 540px; /* Made the cards taller */
        ...
        .drink-card-front {
            ...
            background-size: cover;
            background-position: center;
            /* Cleaner, thinner border matching the app's turquoise */
            border: 1px solid #2dd4bf;
            /* Removed inset shadow, letting the .drink-card-inner shadow work */
            box-shadow: none;
        }
        .card-header-front {
            position: relative; /* For favorite button positioning */
            /* Increased padding, especially on right for button */
            padding: 1rem 4rem 1rem 1.25rem; 
            background: transparent; 
            /* Cleaner, lighter border */
        ...
        /* NEW rule for the relocated tagline */
        .card-hook-tagline {
            text-align: center; 
            ...
            margin-top: auto; /* Pushes it to the bottom of the flex content area */
        }
    ...
    <script>
        ...
        function renderMenu(itemsToRender) {
            ...
            itemsToRender.forEach((item, index) => {
                ...
                // --- MODIFICATION START ---
                // I've moved the favorite button inside the header, changed the flavor profile classes,
                // and adjusted the flavor profile's wrapper div.
                const cardHtml = `
                    <div class="drink-card ${isUnavailable ? 'is-unavailable' : ''}" data-name="${item.name}">
                        <div class="drink-card-inner">
                            <div class="drink-card-front">
                                ...
                                <div class="px-6 pt-4 text-center"> <!-- Reduced pt-5 to pt-4 -->
                                    <p class="text-xl mb-3 font-cinzel text-gray-300">${item.flavorProfile}</p> <!-- Reduced mb-4 to mb-3 -->
                                </div>
                                ...
                                <!-- MOVED THE TAGLINE HERE -->
                                <div class="card-hook-tagline">${item.cardHook}</div>
                                <!-- The card-footer is now just an empty spacer -->
        ...
        function handleJourney() {
            ...
            const cardsHtml = journeyOptions.map((item, index) => {
                // Using the new card front HTML structure for consistency
                const cardInnerHtml = `
                    <div class="drink-card-inner">
                        <div class="drink-card-front">
                            <div class="card-header-front">
                                <h3 class="drink-card-title" style="font-family: 'Rye', cursive; color: ${item.titleColor};">${item.name}</h3>
                            </div>
                            <div class="recipe-content p-2">
                                <ul class="recipe-list-front">${item.recipe.map(r => { const ingName = parseIngredientName(r) || r; const ingInfo = getIngredientInfo(ingName); return `<li><i class="fas ${ingInfo.icon} ${ingInfo.colorClass}"></i><span>${r}</span></li>`; }).join('')}</ul>
                            </div>
                            <div class="card-footer" style="background:rgba(0,0,0,0.5); border-color: #854d0e; color: #f59e0b; text-shadow: 0 0 6px rgba(245, 158, 11, 0.6);">Find in Deck</div>
                        </div>
                    </div>`;
